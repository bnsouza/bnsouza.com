...
export function Navbar({className, ...props}: React.ComponentPropsWithoutRef<"nav">) {
  return <nav {...props} className={clsx(className, "flex flex-1 items-center gap-2 py-1")} />;
}
...
export function NavbarSection({className, ...props}: React.ComponentPropsWithoutRef<"div">) {
  ...
  return (
    <LayoutGroup id={id}>
      <div {...props} className={clsx(className, "flex items-center gap-1")} />
    </LayoutGroup>
  );
}
...
export const NavbarItem = forwardRef(function NavbarItem(
  ...
  let classes = clsx(
    // Base
    "relative flex min-w-0 items-center gap-3 rounded-full px-3 py-2 text-left text-base/6 font-medium text-zinc-950 sm:text-sm/5",
    ...
    <span className={clsx(className, "relative")}>
      {current && (
        <motion.span
          ...
          className="absolute inset-x-2 -bottom-1 h-0.5 rounded-full bg-zinc-950 dark:bg-white"
        />
      )}
      {"href" in props ? (
        <Link
